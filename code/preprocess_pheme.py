...
def load_dataset():
    ...
    path_to_dataset = os.path.join('pheme_dataset', 'all-rnr-annotated-threads')
    ...
    #     extract_conversation(path_to_dataset,tweetid_text, tweetid_owner,userid_tweetid,commentid_srcid)
    ...
    for nfold, fold in enumerate(folds):
        path_to_rumour = os.path.join(path_to_dataset, fold) # rumour/non-rumour
        rumour_not = sorted(os.listdir(path_to_rumour))
        newfolds = [i for i in rumour_not if i[0] != '.']
        rumour_not = newfolds

        # for fld in rumour_not:
        fld = 'rumours'
        if fld == 'rumours': # only select rumours, quit the non-rumour folder
            path_to_tweets = os.path.join(path_to_rumour,fld) # tweet id
            tweet_data = sorted(os.listdir(path_to_tweets))
    ...
    for srcid in train_srcid_replyid.keys():
        claim = tweetid_text[srcid]

    print(len(tweetid_text.keys()),len(tweetid_owner.keys()),len(train_srcid_replyid.keys()),len(test_srcid_replyid.keys()))
    return tweetid_text, tweetid_owner, userid_tweetid, commentid_srcid
