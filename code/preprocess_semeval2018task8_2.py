...
def preprocess_data():
    ...
    srcid_commentid = {}
    for commendid, srcid in commentid_srcid.items():
        if srcid in srcid_commentid:
            srcid_commentid[srcid].append(commendid)
        else:
            srcid_commentid[srcid] = [commendid]
    ...
    for sset in whichset:
        ...
        for tweetid in dataset[sset]:
            if tweetid in commentid_srcid.keys():  # a comment tweet
                ...
                if claimid not in used_claimid:
                    clmVec = text2vec(claim)
                    if sset == 'train':
                        blabel = b_train[claimid]
                    elif sset == 'dev':
                        blabel = b_dev[claimid]
                    elif sset == 'test':
                        blabel = b_test[claimid]
                    b_label = convert_b_label(blabel)

                    if len(clmVec) > 0 and b_label < 2:
                        all_commentids = srcid_commentid[claimid]
                        comVecs = []
                        alabels = []
                        for commentid in all_commentids:
                            comment = tweetid_text[commentid]
                            comVec = text2vec(comment )#+ EOS_TOKEN

                            if sset == 'train':
                                alabel = a_train[commentid]
                            elif sset == 'dev':
                                alabel = a_dev[commentid]
                            elif sset == 'test':
                                alabel = a_test[commentid]
                            a_label = convert_a_label(alabel)

                            if len(comVec) > 0 and a_label < 2:
                                comVecs += comVec
                    ...
                    used_claimid.append(claimid)

        file.close()
        print("{} dataset has {} true claims and {} false claims".format(sset, count_true, count_false))

        # save to files
        np.save(os.path.join(path_to_saved_data, str(sset) + '_claim2'), claim_list)
        np.save(os.path.join(path_to_saved_data, str(sset) + '_comment2'), comment_list)
        np.save(os.path.join(path_to_saved_data, str(sset) + '_alabel2'), alabel_list)
        np.save(os.path.join(path_to_saved_data, str(sset) + '_blabel2'), blabel_list)
        # np.save(os.path.join(path_to_saved_data, str(sset) + '_userid2'), user_list)
    all_file.close()
